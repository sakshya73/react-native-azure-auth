...
export default class Auth {
    constructor(options = {}) {
        ...
        const defaultRedirectUri = `${bundleIdentifier.toLowerCase()}://${bundleIdentifier.toLowerCase()}/${
            Platform.OS
        }/callback`
    ...
    async acquireTokenSilent(parameters = {}) {
        ...
        try {
            let accessToken = await this.cache.getAccessToken(
                input.userId,
                scope
            )
            ...
            if (refreshToken) {
                ...
                if (tokenResponse && tokenResponse.accessToken) {
                    accessToken = await this.cache.saveAccessToken(
                        tokenResponse
                    )
                    return accessToken
                }
